...
namespace ApiPeek.Service
{
    public sealed class ApiPeekService
    {
        ...
        internal static async Task<StorageFile> PeekAndLogInternal()
        {
                ...
                using (Stream fileStream = await file.OpenStreamForWriteAsync())
                {
                    using (ZipArchive zip = new ZipArchive(fileStream, ZipArchiveMode.Create, true))
                    {
                        foreach (Assembly assembly in assemblies)
                        {
                            ...
                            ZipArchiveEntry telemetryFile 
                                = zip.CreateEntry(asmName + ".json", 
            ...
            {
                Debug.WriteLine("[ex] PeekAndLogInternal error: " + ex.Message);
                return null;
            }
        }
        ...
        private static IEnumerable<TypeInfo> GetDefinedTypes(Assembly assembly)
        {
            ...
            catch (ReflectionTypeLoadException ex)
            {
                ...
                return ex.Types.Where(t => t != null)
                    .Select(t => t.GetTypeInfo()).Where(ti => ti != null);
            }
            catch (Exception ex2)
            {
                Debug.WriteLine(ex2);
        ...
        private static void WriteTypes(string name, Func<TypeInfo, bool> test, 
            Action<TypeInfo, JsonWriter> action,
            Assembly assembly, JsonWriter jw)
        {
            Debug.WriteLine(" WriteTypes: " + name);
            //TypeInfo[] types = assembly.DefinedTypes.Where(t => t.IsPublic && test(t)).ToArray();
            TypeInfo[] types = GetDefinedTypes(assembly)
                .Where(t => t.IsPublic && test(t)).ToArray();
        ...
        private static void GetClass(TypeInfo clss, JsonWriter jw)
        {
            ...
            if (clss.BaseType != objType && clss.BaseType.GetTypeInfo().IsPublic) 
        ...
        {
            Type[] interfaces = type.ImplementedInterfaces
                .Where(i => i.GetTypeInfo().IsPublic).ToArray();
        ...
        private static void GetConstructors(TypeInfo type, JsonWriter jw)
        {
            ConstructorInfo[] constructors = 
                type.DeclaredConstructors.Where(InterestingName).ToArray();
        ...
        {
            return (p.GetMethod != null && p.GetMethod.IsStatic) 
                || (p.SetMethod != null && p.SetMethod.IsStatic);
        }

        private static bool EventIsStatic(EventInfo e)
        {
            return (e.AddMethod != null && e.AddMethod.IsStatic) 
                || (e.RemoveMethod != null && e.RemoveMethod.IsStatic);
        }
